...
const Form = () => {
  const currEmployee = useSelector((state) =>
    state.ids ? state.employees.find((e) => e._id === state.ids) : null
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (currId) {
      setError(dispatch(updateEmployee(currId, employee)).resolve);
    } else {
      setError(dispatch(createEmployee(employee)).resolve);
    }
